...
- Optional managed project rule injection behind a workspace checkbox, disabled by default
- A workspace checkbox to add only the generated blocker scripts and optional managed project rule path to `.gitignore`
...
- The managed project rule is installed only when the workspace rule checkbox is enabled.
- Turning the rule checkbox off deletes only `.cursor/rules/cursor-subagent-toggle.mdc` when it still matches the extension-managed content.
...
- Global `OFF` writes the global deny hooks only. It installs workspace managed rules only for folders where the optional rule checkbox is enabled.
...
- `Missing`: The managed project rule is missing while the optional rule checkbox is enabled and the effective state is blocked.
...
When a workspace needs prompt-context protection and the rule checkbox is enabled, the extension creates only this file:
...
- subagent 비활성화 시 더 강한 prompt-context 지시를 위한 managed project rule은 기본 비활성 workspace 체크박스 옵션으로 제공
- 생성된 blocker script들과 선택 managed project rule 경로만 `.gitignore`에 추가하는 workspace 체크박스 제공
...
- managed project rule은 workspace rule 체크박스가 켜져 있을 때만 설치
- rule 체크박스를 끄면 `.cursor/rules/cursor-subagent-toggle.mdc`가 extension 관리 내용과 일치할 때만 삭제
...
- 전역 `OFF`: 전역 deny hooks만 적용. workspace managed rule은 해당 폴더의 선택 rule 체크박스가 켜져 있을 때만 설치
...
- `없음`: 선택 rule 체크박스가 켜져 있고 최종 상태가 차단인데 managed project rule이 없음
...
워크스페이스에 prompt-context 보호가 필요하고 rule 체크박스가 켜져 있을 때 extension은 아래 파일 하나만 생성합니다.
